...
// FlatBuffers binary — replaces all JSON builders
...
private:
    ...
    // ================================================================
    // BUILD POSITIONS FLATBUFFER — replaces buildPositionsJSON
    // ================================================================
    flatbuffers::DetachedBuffer buildPositionsFB(
        py::list   positions,
        py::object account)
    {
        std::vector<FBB::RawPosition> raw_positions;
        raw_positions.reserve(py::len(positions));

        for (auto& item : positions) {
            py::dict p = item.cast<py::dict>();
            FBB::RawPosition rp;
            rp.ticket        = p["ticket"].cast<int64_t>();
            rp.symbol        = p["symbol"].cast<std::string>();
            rp.type          = p["type"].cast<int>();
            rp.volume        = p["volume"].cast<double>();
            rp.open_price    = p["open_price"].cast<double>();
            rp.current_price = p["current_price"].cast<double>();
            rp.sl            = p["sl"].cast<double>();
            rp.tp            = p["tp"].cast<double>();
            rp.profit        = p["profit"].cast<double>();
            rp.swap          = p["swap"].cast<double>();
            rp.commission    = p["commission"].cast<double>();
            rp.open_time     = p["open_time"].cast<int64_t>();
            raw_positions.push_back(rp);
        }

        FBB::RawAccount raw_account{};
        FBB::RawAccount* account_ptr = nullptr;

        if (!account.is_none()) {
            py::dict a    = account.cast<py::dict>();
            raw_account.balance      = a["balance"].cast<double>();
            raw_account.equity       = a["equity"].cast<double>();
            raw_account.margin       = a["margin"].cast<double>();
            raw_account.free_margin  = a["free_margin"].cast<double>();
            raw_account.margin_level = a["margin_level"].cast<double>();
            raw_account.leverage     = a["leverage"].cast<int32_t>();
            account_ptr = &raw_account;
        }

        return FBB::buildPositionsUpdate(raw_positions, account_ptr);
    }
...
public:
            ...
            connector.attr("on_positions_update") =
                    ...
                    py::object account)
                {
                    try {
                        ...
                        // ── Build FlatBuffer while GIL held ──
                        auto buf = buildPositionsFB(
                            pos_list, account
                        );

                        py::gil_scoped_release release;
                        if (positions_cb)
                            positions_cb(std::move(buf));
                    } catch (...) {}
                }
            );
                ...
                [this](py::object status) {
                    try {
                        py::dict d =
                            status.cast<py::dict>();
                        bool connected =
                            d["mt5_connected"].cast<bool>();
                        std::string text =
                            d["status_text"].cast<std::string>();
                        py::gil_scoped_release release;
                        if (connection_cb)
                            connection_cb(connected, text);
                    } catch (...) {}
                }
            );
    ...
    // ================================================================
    // AUTO DETECT SYMBOL — synchronous, reconnect only
    // ================================================================
    ...
    // ================================================================
    flatbuffers::DetachedBuffer checkConnection() {
        if (!initialized) {
            return FBB::buildConnectionStatus(
                false, "Bridge not initialized"
            );
        }
        try {
            py::gil_scoped_acquire gil;
            ...
            std::string text =
                d["status_text"].cast<std::string>();
            return FBB::buildConnectionStatus(connected, text);
        } catch (...) {
            return FBB::buildConnectionStatus(false, "Error");
        }
    }

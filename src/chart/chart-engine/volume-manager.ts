...
export class VolumeManager {
    ...
    private convertOHLCToVolumeData(candles: OHLCData[]): HistogramData[] {
        return candles
            .filter(c => c && c.time && c.volume !== undefined)
            .map(c => this.convertCandleToVolumeCandle(c));
    }

    // ── Cast time to any — OHLCData.time is number, HistogramData expects Time ──
    ...
    private emitVolumeUpdate(candle: OHLCData): void {
        ...
        if (this.onVolumeUpdate) {
            this.onVolumeUpdate(candle.volume, candle.close >= candle.open);
        }
    }
    ...
    public getCurrentVolume(): number | null           { return this.lastVolumeCandle?.volume ?? null; }
